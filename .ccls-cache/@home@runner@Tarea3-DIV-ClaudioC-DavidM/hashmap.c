...
parMap * createPair( char * key,  void * value) {
    parMap * new = (parMap *)malloc(sizeof(parMap));
    new->key = key;
    new->value = value;
    return new;
}
...
void enlarge(HashMap * map) {
    ...
    parMap** aux = map->buckets;
    long sizeAux = map->capacity;
    map->capacity *= 2;
    map->buckets = (parMap **) calloc(map->capacity,sizeof(parMap *));
